...
class WPF_Monday_Admin {
	...
	/**
	 * Loads Monday connection information on settings page
	 *
	 * @access  public
	 * @since   1.0
	 */
	...
	public function register_connection_settings( $settings, $options ) {
		...
		$new_settings['monday_api_key'] = array(
			'title'       => __( 'API Key', 'wp-fusion' ),
			'desc'        => __( 'Enter your Monday API key. You can find your API key in your Monday.com account under Profile > Admin > API.', 'wp-fusion' ),
			'type'        => 'api_validate',
			'section'     => 'setup',
			'class'       => 'api_key',
			'post_fields' => array( 'monday_api_key' )
		);
	...
	/**
	 * Loads standard Monday field names and attempts to match them up with standard local ones
	 *
	 * @access  public
	 * @since   1.0
	 */
	...
	/**
	 * Loads Monday specific settings fields
	 *
	 * @access  public
	 * @since   1.0
	 */
	...
	public function register_settings( $settings, $options ) {
		...
		$site_tracking['site_tracking_header'] = array(
			'title'   => __( 'Monday Site Tracking', 'wp-fusion' ),
			'desc'    => sprintf( __( 'Monday doesn\'t support site tracking.', 'wp-fusion' ) ),
			'std'     => 0,
			'type'    => 'heading',
			'section' => 'main'
		);
	...
	/**
	 * Puts a div around the Monday configuration section so it can be toggled
	 *
	 * @access  public
	 * @since   1.0
	 */
	...
	public function test_connection() {
		...
		$api_key = sanitize_text_field( $_POST['monday_api_key'] );
		...
		if ( is_wp_error( $connection ) ) {
			...
			$options                          = array();
			$options['monday_api_key']        = $api_key;
