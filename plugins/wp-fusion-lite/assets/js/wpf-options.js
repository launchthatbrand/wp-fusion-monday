jQuery(document).ready(function ($) {
  ...
  if ($("body").hasClass("settings_page_wpf-settings")) {
    ...
    $("#import-users-btn").on("click", function () {
      ...
      if (
        $('[name^="wpf_options[import_users]"] option:selected').length == 0
      ) {
        $('[name^="wpf_options[import_users]"]').next().addClass("error");
        setTimeout(function () {
          $('[name^="wpf_options[import_users]"]').next().removeClass("error");
        }, 1000);
        return;
      }
    ...
    var getBatchStatus = function (total, title) {
      if ($("#wpf-batch-status").hasClass("hidden")) {
        ...
        if (total == 0 || isNaN(total)) {
          ...
          $("#wpf-batch-status span.status").html(
            wpf_ajax.strings.processing + " " + total + " " + title
          );
      ...
      $.post(ajaxurl, data, function (response) {
        ...
        if (errors > 0) {
          misc =
            ...
            response.errors +
            " " +
            wpf_ajax.strings.batchErrorsEncountered;
        }
        ...
        if (remaining == 0 || isNaN(remaining)) {
          ...
          $("#wpf-batch-status #cancel-batch").remove();
          $("#wpf-batch-status span.status").html(
            wpf_ajax.strings.batchOperationComplete
          );
          $("#wpf-batch-status")
            .delay(3000)
            .queue(function () {
        ...
        setTimeout(function () {
          ...
          if (completed > 0) {
            ...
            $("#wpf-batch-status span.status").html(
              wpf_ajax.strings.processing + " " + remaining + " " + misc
            );
          }
    ...
    var startBatch = function (button, action, args = false) {
      ...
      button.html(
        '<span class="dashicons dashicons-update-alt wpf-spin"></span>' +
          wpf_ajax.strings.beginningProcessing.replace(
            "ACTIONTITLE",
            action.title
          )
      );
    ...
    $("#export-btn").on("click", function () {
      ...
      var args = {
        skip_processed: $("input[name=skip_already_processed]:checked").val(),
      };
    ...
    $('.wpf-export-option input[type="radio"]').on("change", function () {
      if (parseInt($(this).attr("process_again")) !== 1) {
        $('.skip-processed-container input[type="checkbox"]').prop(
          "checked",
          true
        );
      ...
      $(".skip-processed-container label").html(
        $(".skip-processed-container label")
          .html()
          .replace("[placeholder]", option_title)
      );

      $(".skip-processed-container i").tipTip({
        content: $(".skip-processed-container i")
          .attr("data-tip")
          .replace("[placeholder]", option_title),
        fadeIn: 50,
        fadeOut: 50,
    ...
    $("a#test-connection, a#header-resync").on("click", function () {
      console.log("a#test-connection clicked");
    ...
    $("#dynamics_365_rest_url").on("input", function (event) {
      ...
      if (
        host.slice(Math.max(host.length - 2, 0)).join(".") != "dynamics.com"
      ) {
        $("a#dynamics-365-auth-btn")
          .removeClass("button-primary")
          .addClass("button-disabled");
    ...
    $("[data-unlock]").on("change", function () {
      ...
      if (typeof ischecked == "undefined") {
        ...
        $.each(targets, function (index, target) {
          ...
          $('[id*="' + target + '"]').prop("disabled", function (i, v) {
            return !v;
          });
        });
      }
