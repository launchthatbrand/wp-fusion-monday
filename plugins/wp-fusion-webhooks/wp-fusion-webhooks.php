...
/*
Plugin Name: WP Fusion - Webhooks Addon
Description: Allows sending webhooks from WP Fusion to Zapier or any other third party service.
...
final class WP_Fusion_Webhooks {
	...
	public static function instance() {
		...
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof WP_Fusion_Webhooks ) ) {

			self::$instance = new WP_Fusion_Webhooks;
	...
	private function setup_constants() {
		...
		if(!defined('WPF_WEBHOOKS_DIR_PATH')) {
			define('WPF_WEBHOOKS_DIR_PATH', plugin_dir_path(__FILE__));
		}

		if(!defined('WPF_WEBHOOKS_PLUGIN_PATH')) {
			define('WPF_WEBHOOKS_PLUGIN_PATH', plugin_basename(__FILE__));
		}

		if(!defined('WPF_WEBHOOKS_DIR_URL')) {
			define('WPF_WEBHOOKS_DIR_URL', plugin_dir_url(__FILE__));
		}
	...
	private function includes() {
		...
		if(is_admin()) {
			require_once WPF_WEBHOOKS_DIR_PATH .'includes/admin/class-admin.php';
		}

		require_once WPF_WEBHOOKS_DIR_PATH .'includes/class-public.php';
...
function wp_fusion_webhooks() {

	if ( ! function_exists( 'wp_fusion' ) )
		return;

	return WP_Fusion_Webhooks::instance();

}

add_action( 'plugins_loaded', 'wp_fusion_webhooks', 100 );
